<template>
  <div v-if="visible">
    <div class="settings__setting-title title is-3">
      ...
      <div>
        <plugin-toggle
          :is-enabled="config.workflowOptions.isEnabled"
          :toggled="toggleEnabled"
        />
        <button
          v-if="config.workflowOptions.isEnabled"
          class="button"
          @click="resetAll"
        >
    ...
    <p
      ...
      v-html="translations.workflowSettingsDescription"
    ></p>
      ...
      <div class="settings__setting-content-item box">
        ...
        <div class="table-container">
          <table
            ...
            v-if="config.workflowOptions.workflows.length > 0"
          >
            <thead>
              <tr>
            ...
            <tbody>
              <tr
                v-for="(workflow, index) in config.workflowOptions.workflows"
                v-bind:key="workflow"
              >
                <td>{{ workflow.name }}</td>
                ...
                <td class="has-text-centered">
                  <i
                    v-if="workflow.needsUserConfirmationBeforeExecution"
                    class="fas fa-check"
                  ></i>
                </td>
                <td>
                ...
                <td>
                  <div
                    v-for="executionStep in workflow.executionSteps"
                    class="tags has-addons is-marginless"
                    v-bind:key="executionStep"
                  >
                    <span
                      class="tag"
                      :class="
...
export default Vue.extend({
  ...
  methods: {
    ...
    editWorkflow(index: number) {
      const { config }: { config: UserConfigOptions } = this;
      const workflow: Workflow = deepCopy(
        config.workflowOptions.workflows[index]
      );
      vueEventDispatcher.$emit(
        VueEventChannels.openWorkflowEditingModal,
        workflow,
