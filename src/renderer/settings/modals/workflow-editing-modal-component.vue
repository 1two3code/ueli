        ...
        <div class="message-body">
          ...
          <div class="field">
            ...
            <div class="control">
              <input
                class="is-checkradio"
                id="userConfirmationCheckbox"
                type="checkbox"
                name="userConfirmationCheckbox"
                v-model="workflow.needsUserConfirmationBeforeExecution"
              />
              <label for="userConfirmationCheckbox"></label>
              <div class="field">
                <input
                  class="is-checkradio is-block is-success"
                  id="userConfirmationCheckbox"
                  type="checkbox"
                  name="userConfirmationCheckbox"
                  checked="checked"
          ...
          <div class="field">
            <label class="label">{{
              translations.workflowExecutionSteps
            }}</label>
          </div>
          <div
            class="field has-addons"
            v-for="(executionStep, index) in workflow.executionSteps"
            v-bind:key="executionStep.executionArgumentType"
          >
            <div class="control">
              <button
                disabled
                class="button"
                :class="
                  getExecutionArgumentTypeClass(
                    executionStep.executionArgumentType
                  )
                "
              >
                <span class="icon">
            ...
            <div class="control is-expanded">
              <input
                readonly
                class="input"
                v-model="executionStep.executionArgument"
              />
            </div>
            <div class="control">
              <button
                class="button is-danger"
                @click="removeExecutionStep(index)"
              >
                <span class="icon">
          ...
          <div class="field has-addons">
            <div class="control">
              <div class="select">
                <select
                  v-model="newWorkflowExecutionStep.executionArgumentType"
                  @change="onNewExecutionStepTypeChange"
                >
                  <option
                    v-for="executionArgumentType in executionArgumentTypes"
                    :value="executionArgumentType"
                    v-bind:key="executionArgumentType"
                  >
                    {{
                      getTranslationForExecutionArgumentType(
                        executionArgumentType
                      )
                    }}
                  </option>
                </select>
            ...
            <div
              class="control"
              v-if="
                newWorkflowExecutionStep.executionArgumentType ===
                  executionArgumentTypeFilePath
              "
            >
              <button class="button" @click="openFile">
                <span
                  class="icon tooltip"
                  :data-tooltip="translations.chooseFile"
                >
            ...
            >
              <button class="button" @click="openFolder">
                <span
                  class="icon tooltip"
                  :data-tooltip="translations.chooseFolder"
                >
          ...
          <tags-editing
            :tags="workflow.tags"
            :field-title="translations.workflowTags"
...
import {
  isValidExecutionStep,
  isValidWorkflow
} from "../../../main/plugins/workflow-plugin/workflow-helpers";
...
export default Vue.extend({
  ...
  data() {
    return {
      ...
      executionArgumentTypes: Object.values(
        WorkflowExecutionArgumentType
      ).sort(),
  ...
  methods: {
    addExecutionStep() {
      const { workflow } = this;
      const executionStep: WorkflowExecutionStep = this
        .newWorkflowExecutionStep;
    ...
    getNewWorkflowExecutionStepPlaceholder(): string {
      const { translations }: { translations: TranslationSet } = this;
      const { newWorkflowExecutionStep } = this;
