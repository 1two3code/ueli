  ...
  <li @click="showSetting">
    <a :class="{ 'is-active': isActive }">
      <div class="settings__sidebar-name-container">
        <span
          v-if="getItemIcon(item).length > 0"
          class="icon"
          v-html="getItemIcon(item)"
        ></span>
        <span class="settings__sidebar-name">
          {{ getItemName(item) }}
        </span>
        <span
          v-if="isPluginSetting(item)"
          class="is-size-7"
          :class="{
            'has-text-success': isEnabled(item),
            'has-text-danger': !isEnabled(item)
          }"
        >
...
export default Vue.extend({
  ...
  methods: {
    ...
    isEnabled(
      item: GeneralSettings | PluginSettings | SettingOsSpecific
    ): boolean {
    ...
    isPluginSetting(
      ...
      const allPluginSettings = [
        ...
        ...Object.values(SettingOsSpecific).map(setting =>
          setting.toString().replace(`${platform()}:`, "")
        )
      ];
