<template>
  <div v-if="isVisible">
    <div class="settings__setting-title title is-3">
      ...
      <div>
        <plugin-toggle
          :is-enabled="config.simpleFolderSearchOptions.isEnabled"
    ...
    <p
      ...
      v-html="translations.simpleFolderSearchDescription"
    ></p>
    <div class="settings__setting-content">
              ...
              <div class="field has-addons has-addons-right vertical-center">
                <div class="control">
                  <input
                    ...
                    checked="checked"
                    v-model="config.simpleFolderSearchOptions.showFullFilePath"
      ...
      <div
        v-if="!config.simpleFolderSearchOptions.isEnabled"
      ...
      <div class="settings__setting-content-item box">
        <div class="table-container">
          <table
            ...
            v-if="config.simpleFolderSearchOptions.folders.length > 0"
          >
            <thead>
              <tr>
            ...
            <tbody>
              <tr
                v-for="(folder, index) in config.simpleFolderSearchOptions
                  .folders"
                v-bind:key="folder.folderPath"
              >
                <td>{{ folder.folderPath }}</td>
                <td class="has-text-centered">
                  <i v-if="folder.recursive" class="fas fa-check"></i>
                </td>
                <td class="has-text-centered">
                  <i v-if="folder.excludeHiddenFiles" class="fas fa-check"></i>
                </td>
                <td class="has-text-centered">
...
import {
  defaultSimpleFolderSearchOptions,
...
export default Vue.extend({
  ...
  methods: {
    ...
    resetAll() {
      ...
      const userConfirmationDialogParams: UserConfirmationDialogParams = {
        callback: () => {
          const { config }: { config: UserConfigOptions } = this;
          config.simpleFolderSearchOptions = deepCopy(
            defaultSimpleFolderSearchOptions
          );
          this.updateConfig(true);
        },
    ...
    toggleEnabled() {
      const { config }: { config: UserConfigOptions } = this;
      config.simpleFolderSearchOptions.isEnabled = !config
        .simpleFolderSearchOptions.isEnabled;
      this.updateConfig(true);
    },
  ...
  mounted() {
    ...
    vueEventDispatcher.$on(
      ...
      (
        folderOptions: SimpleFolderSearchFolderOption,
