<template>
  <div v-if="visible">
    <div class="settings__setting-title title is-3">
      ...
      <div>
        <plugin-toggle
          :is-enabled="config.applicationSearchOptions.enabled"
          :toggled="toggleEnabled"
        />
        <button
          v-if="config.applicationSearchOptions.enabled"
          class="button"
          @click="resetAll"
        >
    ...
    <p
      ...
      v-html="translations.applicationSearchSettingsDescription"
    ></p>
    <div class="settings__setting-content">
      ...
      <div class="settings__setting-content-item box">
        <div class="settings__setting-content-item-title">
          ...
          <button class="button" @click="resetApplicationFoldersToDefault">
            <span class="icon"><i class="fas fa-undo-alt"></i></span>
          </button>
        </div>
        <div class="table-container">
          <table
            v-if="config.applicationSearchOptions.applicationFolders.length > 0"
            ...
            <thead>
              <tr>
                ...
                <th class="has-text-right">
                  {{ translations.applicationSearchSettingsRemoveAction }}
                </th>
              </tr>
            </thead>
            <tbody>
              <tr
                v-for="applicationFolder in config.applicationSearchOptions
                  .applicationFolders"
                v-bind:key="applicationFolder"
              >
                <td>{{ applicationFolder }}</td>
                <td class="has-text-right">
                  <button
                    class="button is-danger"
                    @click="removeApplicationFolder(applicationFolder)"
                  >
                    <span class="icon">
      ...
      <div class="settings__setting-content-item box" v-if="isWindows">
        <div class="settings__setting-content-item-title">
          ...
          <button
            class="button"
            @click="resetApplicationFileExtensionsToDefault"
          >
            <span class="icon"><i class="fas fa-undo-alt"></i></span>
          </button>
        </div>
        <div class="table-container">
          <table class="table is-striped is-fullwidth">
            ...
            <tbody>
              <tr
                v-for="applicationFileExtension in config
                  .applicationSearchOptions.applicationFileExtensions"
                v-bind:key="applicationFileExtension"
              >
                ...
                <td class="has-text-right">
                  <button
                    class="button is-danger"
                    @click="
                      removeApplicationFileExtension(applicationFileExtension)
                    "
                  >
                    <span class="icon">
                      <i class="fas fa-trash"></i>
        ...
        <div>
          <button class="button is-success" @click="onAddFileExtensionClick">
            <span class="icon"><i class="fas fa-plus"></i></span>
            <span>{{
              translations.applicationSearchSettingsAddApplicationFileExtension
            }}</span>
          </button>
        </div>
      </div>
        ...
        <div class="settings__options-container">
          <div class="settings__option">
            <div class="settings__option-name">
              {{ translations.showFullFilePath }}
            </div>
            <div class="settings__option-content">
              <div class="field has-addons has-addons-right vertical-center">
                <div class="control">
                  <input
                    id="showFullFilePathCheckbox"
                    type="checkbox"
                    name="showFullFilePathCheckbox"
                    class="switch is-rounded is-success"
                    checked="checked"
                    v-model="config.applicationSearchOptions.showFullFilePath"
                    @change="updateConfig(false)"
                  />
          ...
          <div class="settings__option">
            <div class="settings__option-name">
              {{ translations.applicationSearchSettingsUseNativeIcons }}
            </div>
            <div class="settings__option-content">
              <div class="field is-grouped is-grouped-right">
                <div class="control">
                  <input
                    id="useNativeIconsCheckbox"
                    type="checkbox"
                    name="useNativeIconsCheckbox"
                    class="switch is-rounded is-success"
                    checked="checked"
                    v-model="config.applicationSearchOptions.useNativeIcons"
                    @change="updateConfig(true)"
                  />
                  <label for="useNativeIconsCheckbox"></label
                  ><input
                    id="useNativeIconsCheckbox"
                    type="checkbox"
                    name="useNativeIconsCheckbox"
                    class="switch is-rounded is-success"
                    checked="checked"
                    v-model="config.generalOptions.autostart"
                    @change="updateConfig(false)"
                  />
                </div>
              </div>
...
export default Vue.extend({
  ...
  methods: {
    addApplicationFileExtension(applicationFileExtension: string) {
      ...
      if (
        ...
        showNotification(
          `"${applicationFileExtension}" already exists in your list`,
          NotificationType.Info
        );
      } else {
        config.applicationSearchOptions.applicationFileExtensions.push(
    ...
    addApplicationFolder(folderPath: string) {
      ...
      const folderAlreadyExistsInList =
        config.applicationSearchOptions.applicationFolders.find(
          a => a === folderPath
        ) !== undefined;
      if (folderAlreadyExistsInList) {
        showNotification(
          `Folder "${folderPath}" already exists in your list`,
          NotificationType.Info
        );
      } else {
    ...
    onAddFileExtensionClick() {
      vueEventDispatcher.$emit(
        VueEventChannels.openNewApplicationFileExtensionModal
      );
    },
    onAddFolderClick() {
    ...
    toggleEnabled() {
      const { config }: { config: UserConfigOptions } = this;
      config.applicationSearchOptions.enabled = !config.applicationSearchOptions
        .enabled;
      this.updateConfig(true);
    },
    removeApplicationFileExtension(applicationFileExtension: string) {
      ...
      config.applicationSearchOptions.applicationFileExtensions.splice(
        indexToRemove,
        1
      );
      this.updateConfig(true);
    ...
    removeApplicationFolder(applicationFolder: string) {
      const { config }: { config: UserConfigOptions } = this;
      const indexToRemove = config.applicationSearchOptions.applicationFolders.indexOf(
        applicationFolder
      );
      config.applicationSearchOptions.applicationFolders.splice(
        indexToRemove,
        1
      );
      this.updateConfig(true);
    ...
    resetAll() {
      ...
      const userConfirmationDialogParams: UserConfirmationDialogParams = {
        callback: () => {
          const { config }: { config: UserConfigOptions } = this;
          config.applicationSearchOptions = deepCopy(
            defaultApplicationSearchOptions
          );
          this.updateConfig(true);
        },
  ...
  mounted() {
    ...
    vueEventDispatcher.$on(
      VueEventChannels.applicationFolderAdded,
      (folderPath: string) => {
    ...
    vueEventDispatcher.$on(
      VueEventChannels.applicationFileExtensionAdded,
      (applicationFileExtension: string) => {
