<template>
  <div
    ...
    :class="{ 'scroll-disabled': isLoading }"
    :id="containerId"
  >
    <div
      :id="searchResult.id"
    ...
    >
      <div
        class="search-results__item-icon-container"
        :class="{ active: searchResult.active }"
      >
        <div
          class="search-results__item-icon"
          v-html="getIcon(searchResult.icon, searchResult.active)"
        ></div>
      </div>
      <div class="search-results__item-info-container">
        ...
        <div
          ...
          :class="{
            visible:
              searchResult.active ||
              appearance.showDescriptionOnAllSearchResults,
            active: searchResult.active
          }"
        >
      ...
      <div
        v-if="appearance.showSearchResultNumbers"
        class="search-results__item-number-container"
        :class="{ active: searchResult.active }"
      >
...
export default Vue.extend({
  ...
  methods: {
    getActiveSearchResultItem(): SearchResultItemViewModel | undefined {
      ...
      return searchResults.find((s: any): boolean => s.active);
    },
    ...
    scrollIntoView(index: string) {
      ...
      const scrollBehavior = appearanceOptions.smoothScrolling
        ? "smooth"
        : "auto";
        ...
        if (htmlElement !== undefined && htmlElement !== null) {
          ...
          if (outputContainer !== undefined && outputContainer !== null) {
            ...
            if (elementIsOutOfViewBottom) {
              const scrollTo =
                htmlElement.offsetTop - userInput.clientHeight + 30;
              ...
              let scrollTo =
                htmlElement.offsetTop - outputContainer.clientHeight + 50;
              if (scrollTo < 0) {
                scrollTo = 0;
              }
              outputContainer.scrollTo({
  ...
  mounted() {
    ...
    vueEventDispatcher.$on(
      ...
      (updatedSearchResults: SearchResultItem[]) => {
    ...
    vueEventDispatcher.$on(
      ...
      (userInput: string, privileged: boolean, userConfirmed?: boolean) => {
        ...
        if (activeItem && activeItem.originPluginType !== PluginType.None) {
          if (
            activeItem.needsUserConfirmationBeforeExecution &&
            !userConfirmed
          ) {
            vueEventDispatcher.$emit(
              VueEventChannels.userConfirmationRequested
            );
          } else {
            vueEventDispatcher.$emit(
    ...
    vueEventDispatcher.$on(
      ...
      () => {
        ...
        if (
          activeItem &&
          activeItem.supportsOpenLocation &&
          activeItem.originPluginType !== PluginType.None
        ) {
          vueEventDispatcher.$emit(
    ...
    vueEventDispatcher.$on(VueEventChannels.tabPress, () => {
      ...
      if (
        activeItem &&
        activeItem.supportsAutocompletion &&
        activeItem.originPluginType !== PluginType.None
      ) {
        vueEventDispatcher.$emit(
