...
const unsupportedShellRejection = (shell: WindowsShell | MacOsShell) => {
  return Promise.reject(new Error(`Unsupported shell: ${shell.toString()}`));
};

export const macOsCommandLineExecutor = (
  command: string,
  shell: MacOsShell
): Promise<void> => {
...
export const windowsCommandLineExecutor = (
  command: string,
  shell: WindowsShell
): Promise<void> => {
  switch (shell) {
    ...
    case WindowsShell.PowerShellInWT:
      return executeCommand(
        `start wt.exe powershell -NoExit -Command "&${command}"`
      );
    default:
      return unsupportedShellRejection(shell);
  }
