...
export function searchWindowsApplications(
  ...
  return new Promise((resolve, reject) => {
    ...
    const folders = applicationSearchOptions.applicationFolders
      ...
      .join(",");
    const extensionFilter = applicationSearchOptions.applicationFileExtensions
      .map(e => `*${e}`)
      .join(", ");
    ...
    executeCommandWithOutput(command)
      .then((resultOutput: string) => {
        const result: { errors: string[]; applications: string[] } = JSON.parse(
          resultOutput
        );
        if (result.errors && result.errors.length > 0) {
          logger.error(
            `Errors occurred while searching for applications:\n${result.errors.join(
              "\n"
            )}`
          );
        }
...
export function getMacOsApplicationSearcherCommand(
  ...
  return new Promise((resolve, reject) => {
    if (applicationSearchOptions.applicationFolders.length === 0) {
      ...
      executeCommandWithOutput(
        `${getMacOsApplicationSearcherCommand(
          macOsVersion
        )} | egrep "${folderRegex}"`
      )
        .then(data => {
