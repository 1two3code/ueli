...
export default class ElectronStoreFavoriteRepository
  ...
  constructor() {
    ...
    this.favorites =
      this.migrateUserConfigFavorites() ||
  ...
  private migrateUserConfigFavorites(): Favorite[] | undefined {
    ...
    const userConfigFavorites = userConfigStore.get(
      userConfigFavoritesStoreKey
    );
  ...
  public get(searchResultItem: SearchResultItem): Favorite | undefined {
    return this.favorites.find(
      f => f.item.executionArgument === searchResultItem.executionArgument
    );
  }
  ...
  public update(favorite: Favorite): void {
    ...
    for (let i = 0; i < this.favorites.length; i += 1) {
      if (
        this.favorites[i].item.executionArgument ===
        favorite.item.executionArgument
      ) {
