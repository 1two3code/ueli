...
import {
  windowsFileSearcher as powershellFileSearcher,
  macosFileSearcher
} from "../executors/file-searchers";
import {
  searchWindowsApplications,
  searchMacApplications
} from "../executors/application-searcher";
...
export default function getProductionSearchEngine(
  ...
  const executionPlugins: ExecutionPlugin[] = [
    ...
    new ColorConverterPlugin(
      config.colorConverterOptions,
      electronClipboardCopier
    ),
    new DictionaryPlugin(
      config.dictionaryOptions,
      electronClipboardCopier,
      getGoogleDictionaryDefinitions
    )
  ];
  ...
  if (operatingSystem === OperatingSystem.Windows) {
    ...
    searchPlugins.push(
      new UwpPlugin(config.uwpSearchOptions, filePathExecutor, getAllUwpApps)
    );
