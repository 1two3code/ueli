...
export default class SearchEngine {
  ...
  public execute(
    ...
    return new Promise((resolve, reject) => {
      const originPlugin = this.getAllPlugins()
        .filter(plugin => plugin.isEnabled())
        .find(
          plugin => plugin.pluginType === searchResultItem.originPluginType
        );
      ...
      if (originPlugin !== undefined) {
        ...
        reject(
          new Error(
            "Error while trying to execute search result item. No plugin found for this search result item"
          )
        );
      }
    });
  ...
  public openLocation(searchResultItem: SearchResultItem): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      if (originPlugin && this.pluginSupportsOpenLocation(originPlugin)) {
        ...
        reject(
          new Error(
            "Error while trying to open file location. No plugin found for the search result item"
          )
        );
      }
    });
  ...
  public autoComplete(searchResultItem: SearchResultItem): string {
    ...
    throw new Error(
      "Error while trying to autocomplete. No plugin found for the search result item"
    );
  }
  ...
  public refreshAllIndexes(): Promise<void> {
    return new Promise((resolve, reject) => {
      Promise.all(
        this.searchPlugins
          .filter(plugin => plugin.isEnabled())
          .map(plugin => plugin.refreshIndex())
      )
        .then(() => resolve())
        .catch(err => reject(err));
    });
  ...
  public refreshIndexByPlugin(pluginType: PluginType): Promise<void> {
    return new Promise((resolve, reject) => {
      const pluginToRefreshIndexes = this.searchPlugins.find(
      ...
      if (pluginToRefreshIndexes) {
        ...
        reject(new Error(`Plugin ${pluginType.toString()} not found`));
      }
    });
  }
  ...
  public clearCaches(): Promise<void> {
    return new Promise((resolve, reject) => {
      Promise.all(this.searchPlugins.map(plugin => plugin.clearCache()))
        .then(() => resolve())
        .catch(err =>
          reject(new Error(`Error while trying to clear cache: ${err}`))
        );
    });
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      Promise.all(
        this.getAllPlugins().map(plugin =>
          plugin.updateConfig(updatedConfig, translationSet)
        )
      )
        .then(() => resolve())
        .catch(err => reject(err));
  ...
  private getSearchPluginsResult(
    userInput: string
    ...
    return new Promise((resolve, reject) => {
      ...
      Promise.all(pluginPromises)
        .then(pluginsResults => {
          const all =
            pluginsResults.length > 0
              ? pluginsResults.reduce((a, r) => a.concat(r))
              : [];
          ...
          if (this.logExecution) {
            fuseResult.forEach((fuseResultItem: FuseResult) => {
              const favorite = this.favoriteManager
                ...
                .find(
                  f =>
                    f.item.executionArgument ===
                    fuseResultItem.item.executionArgument
                );
          ...
          const sorted = fuseResult.sort(
            (a: FuseResult, b: FuseResult) => a.score - b.score
          );
          const filtered = sorted.map(
            (item: FuseResult): SearchResultItem => item.item
          );
          let sliced = filtered.slice(0, this.config.maxSearchResults);

          if (this.config.blackList && this.config.blackList.length > 0) {
            sliced = this.filterResultsByBlackList(
              sliced,
    ...
    return new Promise((resolve, reject) => {
      if (this.fallbackPlugins.length > 0 && searchResults.length === 0) {
        const fallbackPluginPromises = this.fallbackPlugins
          .filter(fallbackPlugin =>
            fallbackPlugin.isValidUserInput(userInput, true)
          )
          .map(fallbackPlugin =>
            fallbackPlugin.getSearchResults(userInput, true)
          );
          ...
          .then(resultLists => {
            const result =
              resultLists.length > 0
                ? resultLists.reduce((all, resultList) =>
                    all.concat(resultList)
                  )
                : [];
  ...
  private filterResultsByBlackList(
    ...
    return items.filter(item => {
      return blackList.every(
        blackListKeyword =>
          !this.searchResultItemContainsBlackListKeyword(item, blackListKeyword)
      );
    });
  }
  ...
  private searchResultItemContainsBlackListKeyword(
    ...
    return (
      searchResultItem.name
        .toLowerCase()
        .indexOf(blackListKeyword.toLowerCase()) > -1
    );
  }
  ...
  private pluginSupportsOpenLocation(
    plugin: any
  ): plugin is OpenLocationPlugin {
    const openLocationPlugin = plugin as OpenLocationPlugin;
    return openLocationPlugin.openLocation !== undefined;
  }

  private pluginSupportsAutocompletion(
    plugin: any
  ): plugin is AutoCompletionPlugin {
