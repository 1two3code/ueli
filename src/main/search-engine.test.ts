...
import {
  ...
  SearchEngineOptions
} from "../common/config/search-engine-options";
...
describe(SearchEngine.name, () => {
  ...
  it("should return no search results found when searching for something non-existent", done => {
    ...
    searchEngine
      ...
      .then(searchResults => {
        expect(searchResults.length).toBe(1);
        expect(searchResults[0].name).toBe(
          translationSet.noSearchResultsFoundTitle
        );
        done();
      })
      .catch(err => done(err));
  ...
  it("it should allow inprecise search terms when fuzzyness threshold is higher", done => {
    ...
    const config = { ...defaultSearchEngineOptions, ...userOptions };
  ...
  it("should only refresh indexes of enabled plugins", done => {
    ...
    searchEngine
      .refreshAllIndexes()
      .then(() => {
        const refreshedPlugins = fakePlugins.filter(
          plugin => plugin.getIndexRefreshCount() > 0
        );
  ...
  it("should only search entries of enabled plugins", done => {
    ...
    searchEngine
      ...
      .then(searchResultItems => {
        ...
        const expected =
          searchPlugins.filter(plugin => plugin.isEnabled()).length *
          items.length;
