...
import {
  FileIconDataResult,
  getFileIconDataUrl
} from "../../../common/icon/generate-file-icon";
...
export default class SimpleFolderSearchPlugin
  implements SearchPlugin, AutoCompletionPlugin, OpenLocationPlugin {
  ...
  private readonly fileSearcher: (
    options: FileSearchOption
  ) => Promise<string[]>;
    ...
    return new Promise((resolve, reject) => {
      if (this.config.folders.length === 0) {
        ...
        Promise.all(searchPromises)
          .then(resultLists => {
            const results = resultLists.reduce((all, resultList) => {
              return all.concat(resultList);
            });
