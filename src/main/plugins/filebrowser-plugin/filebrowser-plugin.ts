...
export default class FileBrowserExecutionPlugin
  implements ExecutionPlugin, AutoCompletionPlugin, OpenLocationPlugin {
  ...
  private readonly fileIconGenerator: (
    filePath: string,
    fileIconDefault: Icon,
    folderIconDefault: Icon
  ...
  public isValidUserInput(userInput: string, fallback?: boolean): boolean {
    const filePaths = [userInput, dirname(userInput)].filter(
      filePath => filePath !== "."
    );
    return filePaths.some(
      filePath => this.filePathValidator(filePath) && existsSync(filePath)
    );
  }
  ...
  public getSearchResults(
    ...
    return new Promise((resolve, reject) => {
      ...
      const searchTerm = userInputIsExactMatch
        ? undefined
        : basename(userInput);
  ...
  private buildSearchResults(
    ...
    return new Promise((resolve, reject) => {
      ...
      const promises = unsortedResults.map(unsortedResult =>
        this.fileIconGenerator(
          unsortedResult.executionArgument,
          defaultFileIcon,
          defaultFolderIcon
        )
      );
        ...
        .then(iconResults => {
          unsortedResults.forEach(unsortedResult => {
            const icon = iconResults.find(
              iconResult =>
                iconResult.filePath === unsortedResult.executionArgument
            );
            if (icon) {
  ...
  private sortResults(
    unsortedResults: SearchResultItem[],
    searchTerm?: string
  ): SearchResultItem[] {
