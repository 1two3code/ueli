...
export default class UeliCommandSearchPlugin implements SearchPlugin {
  ...
  public getAll(): Promise<SearchResultItem[]> {
    return new Promise(resolve => {
      const result = this.getAllCommands().map(command =>
        this.createSearchResultItemFromUeliCommand(command)
      );
      resolve(result);
    });
  }
  ...
  public execute(searchResultItem: SearchResultItem): Promise<void> {
    return new Promise((resolve, reject) => {
      const ueliCommand = this.getAllCommands().find(
        command =>
          command.executionArgument === searchResultItem.executionArgument
      );
      if (ueliCommand) {
        ...
        reject(
          new Error(
            "Error while trying to execute ueli command: Invalid ueli command"
          )
        );
      }
    });
  ...
  private createSearchResultItemFromUeliCommand(
