...
export default class WebSearchPlugin implements ExecutionPlugin {
  ...
  public getSearchResults(
    ...
    return new Promise((resolve, reject) => {
      ...
      const suggestionWebSearchEngines = webSearchEngines.filter(
        webSearchEngine => {
        ...
        .then(suggestions => {
          ...
          webSearchEngines.forEach(webSearchEngine => {
            results.push({
              ...
              executionArgument: this.buildExecutionArgument(
                webSearchEngine,
                userInput
              ),
  ...
  private buildExecutionArgument(
    webSearchEngine: WebSearchEngine,
    userInput: string
  ): string {
    return this.replaceQueryInUrl(
      this.getSearchTerm(webSearchEngine, userInput),
      webSearchEngine.url
    );
  }
  ...
  private getSuggestionsByWebSearchEngine(
    ...
    return new Promise((resolve, reject) => {
      if (websearchEngine.suggestionUrl && searchTerm) {
        const suggestionUrl = this.replaceQueryInUrl(
          searchTerm,
          websearchEngine.suggestionUrl
        );
