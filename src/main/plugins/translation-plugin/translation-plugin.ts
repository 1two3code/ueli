...
export default class TranslationPlugin implements ExecutionPlugin {
  ...
  public isValidUserInput(userInput: string) {
    return (
      ...
      userInput.replace(this.config.prefix, "").length >=
        this.config.minSearchTermLength
    );
  }
      ...
      LingueeTranslator.getTranslations(url)
        .then(translations => {
          const result = translations.map(
            (t): SearchResultItem => {
              return {
                description: `${capitalize(t.wordType.pos)}`,
