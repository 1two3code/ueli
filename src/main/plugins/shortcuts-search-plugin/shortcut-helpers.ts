...
function isValidShortcutFilePath(
  filePath: string,
  filePathValidator: (path: string) => boolean
): boolean {
...
export function isValidShortcutType(shortcutType: ShortcutType): boolean {
  return (
    Object.values(ShortcutType).find(s => s === shortcutType) !== undefined
  );
}
...
export function isValidShortcutExecutionArgument(
  ...
  switch (shortcut.type) {
    ...
    case ShortcutType.FilePath:
      return isValidShortcutFilePath(
    ...
    default:
      throw new Error(`ShortcutExecutionArgument invalid: ${shortcut.type}`);
  }
}

export function isValidShortcutToAdd(
  shortcut: Shortcut,
  filePathValidator: (filePath: string) => boolean
): boolean {
...
export function getDefaultShortcutIcon(shortcut: Shortcut): Icon {
  switch (shortcut.type) {
    case ShortcutType.CommandlineTool:
