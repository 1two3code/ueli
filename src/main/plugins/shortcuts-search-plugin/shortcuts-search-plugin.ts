...
export default class ShortcutsSearchPlugin
  implements SearchPlugin, OpenLocationPlugin {
  ...
  private readonly filePathExecutor: (
    filePath: string,
    privileged: boolean
  ) => Promise<void>;

  private readonly filePathLocationExecutor: (
    filePath: string
  ) => Promise<void>;
    ...
    return new Promise(resolve => {
      resolve(
        this.config.shortcuts.map(
          (shortcut): SearchResultItem => this.createSearchResultItem(shortcut)
        )
      );
    });
  }
  ...
  public execute(
    ...
    const decodeResult = this.decodeExecutionArgument(
    ...
    switch (decodeResult.shortcutType) {
      ...
      case ShortcutType.FilePath:
        return this.filePathExecutor(
          decodeResult.executionArgument,
          privileged
        );
      case ShortcutType.CommandlineTool:
        return this.commandlineExtecutor(decodeResult.executionArgument);
      default:
        return this.getUnsupportedShortcutTypePromise(
          decodeResult.shortcutType
        );
    }
  }
  ...
  private getUnsupportedShortcutTypePromise(
    shortcutType: ShortcutType
  ): Promise<void> {
    return new Promise((resolve, reject) => {
      reject(new Error(`Unsupported shortcut type: ${shortcutType}`));
    });
  }
  ...
  private encodeExecutionArgument(shortcut: Shortcut): string {
    return `${this.getExecutionArgumentPrefix(shortcut.type)}${
      shortcut.executionArgument
    }`;
  }

  private decodeExecutionArgument(
    executionArgument: string
  ): ExecutionArgumentDecodeResult {
    Object.values(ShortcutType).forEach(s => {
      const shortcutType: ShortcutType = s;
      if (
        executionArgument.startsWith(
          this.getExecutionArgumentPrefix(shortcutType)
        )
      ) {
        return {
          executionArgument: executionArgument.replace(
            this.getExecutionArgumentPrefix(shortcutType),
            ""
          ),
          shortcutType
      ...
      throw new Error(`Execution argument invalid: ${executionArgument}`);
    });
  ...
  private createSearchResultItem(shortcut: Shortcut): SearchResultItem {
    return {
      ...
      name: shortcut.name,
      needsUserConfirmationBeforeExecution:
        shortcut.needsUserConfirmationBeforeExecution,
