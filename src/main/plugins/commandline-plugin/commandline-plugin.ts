...
export default class CommandlinePlugin implements ExecutionPlugin {
  public pluginType = PluginType.Commandline;

  constructor(
    private config: CommandlineOptions,
    private translationSet: TranslationSet,
    private readonly commandlineExecutor: (
      command: string,
  ...
  public getSearchResults(
    ...
    return new Promise((resolve, reject) => {
      ...
      const result: SearchResultItem = {
        description: this.translationSet.commandlineSearchResultDescription.replace(
          "{{command}}",
          command
        ),
