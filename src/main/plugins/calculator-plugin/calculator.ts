...
export default class Calculator {
  public static isValidInput(
    ...
    try {
      ...
      result = mathjs.eval(
        this.normalizeInput(input, decimalSeparator, argumentSeparator)
      );
    } catch (e) {
      return false;
    }
    ...
    return !Number.isNaN(result) || this.isValidMathType(result) || false;
  }
  ...
  public static calculate(
    ...
    const result: string = mathjs
      .eval(this.normalizeInput(input, decimalSeparator, argumentSeparator))
      .toString();
  ...
  private static isValidMathType(input: any): boolean {
    ...
    if (
      (mathType === "Unit" && input.value === null) ||
      mathType === "Function"
    ) {
      return false;
    }
