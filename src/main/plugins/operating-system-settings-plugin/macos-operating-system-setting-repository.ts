...
export default class MacOsOperatingSystemSettingRepository
    ...
    this.getFilePaths()
      .then(filePaths => {
        generateMacAppIcons(filePaths)
          .then(() => {
            this.all = filePaths.map(filePath =>
              this.buildOperatingSystemSetting(filePath)
            );
          })
          .catch(err => {
            this.all = [];
  ...
  ): Promise<OperatingSystemSetting[]> {
    return new Promise(resolve => {
      resolve(this.all);
    });
  }

  private buildOperatingSystemSetting(
    filePath: string
    ...
    return {
      ...
      icon: {
        parameter: getApplicationIconFilePath(filePath),
        type: IconType.URL
      },
