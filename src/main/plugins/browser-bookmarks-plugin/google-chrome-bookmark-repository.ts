...
export default class GoogleChromeBookmarkRepository
  implements BrowserBookmarkRepository {
          ...
          if (exists) {
            FileHelpers.readFile(this.bookmarkFilePath)
              .then(data => {
                ...
                const rootsCombined = Object.values(jsonParsed.roots).reduce(
                  (combined: any, value: any) => {
                    if (value && value.children) {
                      ...
                      combined.children = combined.children.concat(
                        value.children
                      );
                    }
                    return combined;
                ...
                const bookmarks: BrowserBookmark[] = this.getBookmarksFromObject(
                  rootsCombined
                );
                resolve(bookmarks);
              })
              .catch(err =>
                reject(new Error(`Can't read bookmarks file: ${err}`))
              );
          } else {
            reject(
              new Error(
                `Bookmark file (${this.bookmarkFilePath}) does not exist`
              )
            );
          }
        })
    ...
    Object.entries(data).forEach(([key, v]) => {
      ...
      if (key === "children") {
        ...
        folders.forEach((folder: any) => {
          result = result.concat(this.getBookmarksFromObject(folder));
        });
      }
    });
