...
export default class ApplicationIconService {
  private readonly generateIcons: (
  ...
  constructor(
    generateIcons: (applicationFilePaths: string[]) => Promise<void>,
    logger: Logger
  ) {
    ...
    return new Promise((resolve, reject) => {
      ...
      Promise.all(fileExistPromises)
        .then(fileExistResults => {
          const applicationsToGenerateIcons = applications.filter(
            application => {
              const fileExistsResult = fileExistResults.find(
                f =>
                  f.filePath ===
                  getApplicationIconFilePath(application.filePath)
              );
              if (fileExistsResult) {
          ...
          if (applicationsToGenerateIcons.length === 0) {
            this.logger.debug(
              `Skipping app icon generation. All app icons already exist`
            );
            resolve();
          } else {
            this.logger.debug(
            ...
            this.generateIcons(
              applicationsToGenerateIcons.map(
                application => application.filePath
              )
            )
              .then(() => {
                this.logger.debug(
                  `Successfully generated ${applicationsToGenerateIcons.length} icons`
                );
                resolve();
              })
      ...
      FileHelpers.readFilesFromFolder(applicationIconLocation)
        .then(files => {
          ...
          Promise.all(deletionPromises)
            .then(() => {
              this.logger.debug(
                `Deleted ${files.length} app icons in ${applicationIconLocation}`
              );
              resolve();
            })
            .catch(err => reject(err));
