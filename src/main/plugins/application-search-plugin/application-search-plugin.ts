...
export default class ApplicationSearchPlugin
  ...
  private readonly executeApplication: (
    ...
    return new Promise((resolve, reject) => {
      this.applicationRepository
        ...
        .catch(err =>
          reject(
            new Error(
              `Error while trying to clear application repository cache: ${err}`
            )
          )
        );
    });
  ...
  private createSearchResultItemFromApplication(
