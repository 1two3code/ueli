...
export default class ColorConverterPlugin implements ExecutionPlugin {
  ...
  private buildSearchResult(value: string): SearchResultItem[] {
    ...
    if (this.config.hexEnabled) {
      result.push(
        this.buildColorSearchResult("HEX", converted.hex().toString())
      );
    }

    if (this.config.rgbEnabled) {
      result.push(
        this.buildColorSearchResult("RGB", converted.rgb().toString())
      );
    }
    ...
    if (this.config.hslEnabled) {
      result.push(
        this.buildColorSearchResult("HSL", converted.hsl().toString())
      );
    }
  ...
  private buildColorSearchResult(
    colorName: string,
    colorValue: string
  ): SearchResultItem {
    return {
