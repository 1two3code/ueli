...
} from "../common/helpers/operating-system-helpers";
import {
  executeFilePathWindows,
...
const configRepository = new ElectronStoreConfigRepository(
...
const filePathExecutor =
  operatingSystem === OperatingSystem.Windows
    ? executeFilePathWindows
    : executeFilePathMacOs;
const trayIconFilePath =
  operatingSystem === OperatingSystem.Windows
    ? trayIconPathWindows
    : trayIconPathMacOs;
const windowIconFilePath =
  operatingSystem === OperatingSystem.Windows
    ? windowIconWindows
    : windowIconMacOs;
...
let searchEngine = getProductionSearchEngine(
  operatingSystem,
...
function onIndexRefreshStarted() {
  BrowserWindow.getAllWindows().forEach(window =>
    window.webContents.send(IpcChannels.refreshIndexesStarted)
  );
}
...
function refreshAllIndexes() {
  ...
  searchEngine
    ...
    .then(() => {
      ...
      notifyRenderer(
        translationSet.successfullyRefreshedIndexes,
...
function refreshIndexOfPlugin(pluginType: PluginType) {
  ...
  searchEngine
    ...
    .then(() => {
      logger.debug(
        `Successfully refresh index of plugin ${pluginType.toString()}`
      );
      notifyRenderer(
        translationSet.successfullyRefreshedIndexes,
        NotificationType.Info
...
function clearAllCaches() {
  searchEngine
    ...
    .then(() => {
      ...
      notifyRenderer(
        translationSet.successfullyClearedCaches,
        NotificationType.Info
      );
    })
...
function registerGlobalKeyboardShortcut(
  toggleAction: () => void,
  newHotKey: GlobalHotKey
) {
  const validHotKey = isValidHotKey(newHotKey)
    ? newHotKey
    : defaultGeneralOptions.hotKey;
  globalShortcut.unregisterAll();
  globalShortcut.register(
    `${validHotKey.modifier ? `${validHotKey.modifier}+` : ``}${
      validHotKey.key
    }`,
    toggleAction
  );
  ...
  return Math.round(
    Number(
      display.bounds.x +
        ...
        config.appearanceOptions.windowWidth / 2
    )
  );
}
  ...
  if (windowExists(mainWindow)) {
    if (mainWindow.isVisible()) {
      ...
      const windowBounds: Electron.Rectangle = {
        ...
        x:
          config.generalOptions.rememberWindowPosition &&
          lastWindowPosition &&
  ...
  if (windowExists(mainWindow)) {
    ...
    const windowHeight =
      searchResultCount > appearanceOptions.maxSearchResultsPerPage
        ...
        : Math.round(
            Number(searchResultCount) *
              Number(appearanceOptions.searchResultHeight) +
              Number(appearanceOptions.userInputHeight) +
              Number(appearanceOptions.userInputBottomMargin)
          );

    mainWindow.setSize(
      Number(appearanceOptions.windowWidth),
      Number(windowHeight)
    );
    if (center) {
      mainWindow.center();
...
function mainWindowNeedsToBeTransparent(
  ...
  return (
    userConfigOptions.appearanceOptions.allowTransparentBackground === true
  );
}
...
function getMainWindowBackgroundColor(
  ...
  return userConfigOptions.appearanceOptions.allowTransparentBackground === true
    ? transparent
    : toHex(
        userConfigOptions.colorThemeOptions.searchResultsBackgroundColor,
        "#FFFFFF"
      );
}
...
function updateConfig(
  ...
  if (
    updatedConfig.generalOptions.language !== config.generalOptions.language
  ) {
  ...
  if (updatedConfig.generalOptions.hotKey !== config.generalOptions.hotKey) {
    registerGlobalKeyboardShortcut(
      toggleMainWindow,
      updatedConfig.generalOptions.hotKey
    );
  }

  if (
    updatedConfig.generalOptions.rescanIntervalInSeconds !==
    config.generalOptions.rescanIntervalInSeconds
  ) {
    if (rescanInterval) {
      clearInterval(rescanInterval);
    }
  ...
  if (
    ...
    Number(config.appearanceOptions.windowWidth)
  ) {
    mainWindow.resizable = true;
    mainWindow.setSize(
      Number(updatedConfig.appearanceOptions.windowWidth),
  ...
  if (
    JSON.stringify(updatedConfig.appearanceOptions) !==
    JSON.stringify(config.appearanceOptions)
  ) {
    mainWindow.webContents.send(
      IpcChannels.appearanceOptionsUpdated,
      updatedConfig.appearanceOptions
    );
  }

  if (
    JSON.stringify(updatedConfig.colorThemeOptions) !==
    JSON.stringify(config.colorThemeOptions)
  ) {
    if (
      ...
      mainWindow.setBackgroundColor(
        getMainWindowBackgroundColor(updatedConfig)
      );
    }
    mainWindow.webContents.send(
      IpcChannels.colorThemeOptionsUpdated,
      updatedConfig.colorThemeOptions
    );
  }

  if (
    JSON.stringify(updatedConfig.generalOptions) !==
    JSON.stringify(config.generalOptions)
  ) {
    mainWindow.webContents.send(
      IpcChannels.generalOptionsUpdated,
      updatedConfig.generalOptions
    );
  }

  if (
    updatedConfig.generalOptions.allowWindowMove !==
    config.generalOptions.allowWindowMove
  ) {
      ...
      searchEngine
        ...
        .then(() => {
          if (needsIndexRefresh) {
            ...
            notifyRenderer(
              translationSet.successfullyUpdatedconfig,
              NotificationType.Info
            );
          }
        })
...
function registerAllIpcListeners() {
  ...
  ipcMain.on(
    IpcChannels.search,
    (event: Electron.IpcMainEvent, userInput: string) => {
      searchEngine
  ...
  ipcMain.on(
    ...
    (event: Electron.Event, searchResultItem: SearchResultItem) => {
      searchEngine
  ...
  ipcMain.on(
    IpcChannels.autoComplete,
    (event: Electron.IpcMainEvent, searchResultItem: SearchResultItem) => {
  ...
  ipcMain.on(
    IpcChannels.folderPathRequested,
    (event: Electron.IpcMainEvent) => {
      dialog
        .showOpenDialog(settingsWindow, { properties: ["openDirectory"] })
        .then(result =>
          event.sender.send(IpcChannels.folderPathResult, result.filePaths)
        )
  ...
  ipcMain.on(
    IpcChannels.filePathRequested,
    (event: Electron.IpcMainEvent, filters?: Electron.FileFilter[]) => {
      dialog
        .showOpenDialog(settingsWindow, { filters, properties: ["openFile"] })
        .then(result =>
          event.sender.send(IpcChannels.filePathResult, result.filePaths)
        )
  ...
  ipcMain.on(IpcChannels.clearExecutionLogConfirmed, () => {
    searchEngine
      ...
      .then(() =>
        notifyRenderer(
          translationSet.successfullyClearedExecutionLog,
          NotificationType.Info
        )
      )
      .catch(err => {
  ...
  ipcMain.on(
    IpcChannels.selectInputHistoryItem,
    (event: Electron.IpcMainEvent, direction: string) => {
      const newUserInput =
        direction === "next"
  ...
  ipcMain.on(IpcChannels.ueliCommandExecuted, (cmd: unknown) => {
    const command = cmd as UeliCommand;
